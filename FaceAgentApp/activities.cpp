...
A::A() {}
A::~A() {}
void function4(void *param);

void A::function1()
{
    CommonThread *th = new CommonThread;
    CommonThread *th1 = new CommonThread;
    CommonThread *th2 = new CommonThread;

    int *tmp = new int;
    *tmp = 5;
    printf("pointer = %p\n", tmp);

    void *func2Ptr = (void*)&A::function2;

    th->startThread((void* (*) (void*))function4, &tmp, sizeof(tmp));
    sleep(2);

    A * t = new A;

    th2->startVirtualFunctionThread<A>(*t, &A::function2, &tmp, sizeof(tmp));
    sleep(2);

    //if(!threadAcceptConnection->startThread(&, this, sizeof(Network)))
    //if(!threadAcceptConnection->startThread((void* (*) (void*))function, &pThis, sizeof(pThis)))
}
...
void function4(void *param)
{
    ...
    for(int i = 0; i < 8; i++)
    {
        printf("%2x", ((char*)param)[i]);
    }
    printf("\n");

    int *tmp = NULL;
    memcpy(&tmp, param, sizeof(int*));
    printf("int = %d\n", *tmp);
    return;
}
