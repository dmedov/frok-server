...
#define MODULE_NAME         "MODEL_FISHERFACES"

FaceModelFisherfaces::FaceModelFisherfaces()
{
    TRACE_F_T("Do not use this constructor");
}
FaceModelFisherfaces::~FaceModelFisherfaces()
{
    TRACE_N("~FaceModelFisherfaces");
}
FaceModelFisherfaces::FaceModelFisherfaces(std::string userId) : FaceModelAbstract(userId)
{
...
FrokResult FaceModelFisherfaces::GenerateUserModel(std::vector<cv::Mat> &grayFaces)
{
    TRACE_T("started");
    if(grayFaces.empty())
    {
...
FrokResult FaceModelFisherfaces::AddFaceToModel(cv::Mat &grayFace)
{
    TRACE_T("started");
    labels.push_back(0);
...
FrokResult FaceModelFisherfaces::LoadUserModel(const char *userPath)
{
    TRACE_T("started");
    if(userPath == NULL)
    {
    ...
    modelPath.append(USER_MODEL_FILENAME_FISHERFACES);
...
FrokResult FaceModelFisherfaces::SaveUserModel(const char *userPath)
{
    TRACE_T("started");
    if(userPath == NULL)
    {
...
FrokResult FaceModelFisherfaces::GetPredictedFace(cv::Mat &targetFace, cv::Mat &predictedFace)
{
    TRACE_T("started");
    try
    {
